...
#[[
  ...
  Options:
    MSBUILD <TRUE|FALSE> - Use the MSBuild spec for targeting modules
]]
function(module_sources)
  set(options)
  set(oneValueArgs NAME_PREFIX INSTALL_ROOT MSBUILD)
  ...
  if(NOT MS_MSBUILD)
    ...
    if(MS_HEADERS)
      files_to_interface(VAR _public_headers
        BASE_DIRS ${MS_INCLUDE_DIRS}
        INSTALL_DIR "${MS_INSTALL_PREFIX}/include"
        FILES ${MS_HEADERS})
      list(APPEND _public_sources ${_public_headers})
    endif()
    if(MS_HEADER_UNITS)
      files_to_interface(VAR _public_header_units
        BASE_DIRS ${MS_PUBLIC_DIRS}
        INSTALL_DIR "${MS_INSTALL_PREFIX}/include"
        FILES ${MS_HEADER_UNITS})
      list(APPEND _public_sources ${_public_header_units})
    endif()
    if(MS_PUBLIC_MODULES)
      files_to_interface(VAR _public_modules
        BASE_DIRS ${MS_PUBLIC_DIRS}
        INSTALL_DIR "${MS_INSTALL_PREFIX}/public"
        FILES ${MS_PUBLIC_MODULES})
      list(APPEND _public_sources ${_public_modules})
    endif()
    if(MS_PRIVATE_MODULES)
      files_to_interface(VAR _private_modules
        BASE_DIRS ${MS_PRIVATE_DIRS}
        INSTALL_DIR "${MS_INSTALL_PREFIX}/src"
        FILES ${MS_PRIVATE_MODULES})
      list(APPEND _private_sources ${_private_modules})
    endif()
    target_sources(${_ms_target} PUBLIC ${_public_sources})
    target_sources(${_ms_target} PRIVATE ${_private_sources})
  endif()
endfunction()
...
function(get_file_set_install_command_args)
  ...
  ams_log(DEBUG "${CMAKE_CURRENT_FUNCTION}->start: args: '${ARGV}'")
  set(_varname ${ARGV0})
  set(_gfs_out)
  ...
  foreach(_idx RANGE 0 ${_gfs_size})
    ...
    if(_gfs_last_param STREQUAL "FILE_SET")
      ...
      dict(_gfs_out SET KEY "FILE_SET" VALUE ${_gfs_param})
    elseif(_gfs_last_param STREQUAL "DESTINATION")
      list(GET GFS_FILE_SET_ARGS ${_idx} _gfs_param)
      dict(_gfs_out SET KEY "DESTINATION" VALUE ${_gfs_param})
    endif()
    ams_log(DEBUG "${CMAKE_CURRENT_FUNCTION}->loop: DESTINATION: ${_gfs_out}")
  endforeach()
...
#[[
  ...
  Options:
    ...
    INSTALL - If set, parses all instructions from VAR and transforms VAR into a list of commands to pass to install().
...
function(install_arguments_builder)
  ...
  set(options INSTALL)
  set(oneValueArgs VAR TARGETS EXPORT COMPONENT INSTALL_FILE_SET LOG_LEVEL)
  set(multiValueArgs FILE_SET TARGET_TYPES)
  cmake_parse_arguments(IA "${options}" "${oneValueArgs}" "${multiValueArgs}" ${ARGN})
  ams_log(DEBUG "${CMAKE_CURRENT_FUNCTION}->START: args: '${ARGN}'")
  
  if(NOT IA_VAR)
    message(SEND_ERROR "${CMAKE_CURRENT_FUNCTION}->INVALID-ARGUMENTS: VAR not specified")
  endif()
  set(_output "${${IA_VAR}}")
  ams_log(DEBUG "${CMAKE_CURRENT_FUNCTION}->PARSING: VAR: '${_output}'")
  if(_output STREQUAL "")
    dict(_output NEW)
  endif()
  # set or validate TARGET in VAR
  set(_var_target)
  dict(_output GET _var_target ERROR_VARIABLE _json_err KEY TARGETS)
  message(DEBUG "${CMAKE_CURRENT_FUNCTION}->PARSING: TARGETS: '${_var_target}'")
  if(_json_err)
    if(NOT IA_TARGETS)
      ams_log(FATAL_ERROR "${CMAKE_CURRENT_FUNCTION}: TARGETS not specified")
    endif()
    ams_log("output gen target = '${_output}'")
    dict(_output SET ERROR_VARIABLE _json_err KEY TARGETS VALUE ${IA_TARGETS})
    if(_json_err)
      ams_log(FATAL_ERROR "${CMAKE_CURRENT_FUNCTION}: Failed to set TARGETS: ${_json_err}")
    endif()
  endif()
  # set or validate EXPORT in VAR
  set(_var_export)
  dict(_output GET _var_export ERROR_VARIABLE _json_err KEY EXPORT)
  if(_json_err)
    if(NOT IA_EXPORT)
      ams_log(FATAL_ERROR "${CMAKE_CURRENT_FUNCTION}: EXPORT not specified")
    endif()
    dict(_output SET ERROR_VARIABLE _json_err KEY EXPORT VALUE ${IA_EXPORT})
    if(_json_err)
      ams_log(FATAL_ERROR "${CMAKE_CURRENT_FUNCTION}: Failed to set EXPORT: ${_json_err}")
    endif()
  endif()
  # set or validate COMPONENT in VAR
  set(_var_component)
  dict(_output GET _var_component ERROR_VARIABLE _json_err KEY COMPONENT)
  if(_json_err)
    if(NOT IA_COMPONENT)
      ams_log(FATAL_ERROR "${CMAKE_CURRENT_FUNCTION}: COMPONENT not specified")
    endif()
    dict(_output SET ERROR_VARIABLE _json_err KEY COMPONENT VALUE ${IA_COMPONENT})
    if(_json_err)
      ams_log(FATAL_ERROR "${CMAKE_CURRENT_FUNCTION}: Failed to set COMPONENT: ${_json_err}")
    endif()
  endif()
  # set or replace TARGET_TYPES in VAR
  if(IA_TARGET_TYPES)
    set(_valid_target_types ARCHIVE LIBRARY RUNTIME OBJECTS FRAMEWORK BUNDLE
                            PRIVATE_HEADER PUBLIC_HEADER RESOURCE INCLUDES)
    set(_valid_tt_args DESTINATION PERMISSIONS CONFIGURATIONS COMPONENT NAMELINK_COMPONENT
                       OPTIONAL EXCLUDE_FROM_ALL NAMELINK_ONLY NAMELINK_SKIP)
    dict(_target_types NEW)
    set(_tt_name)
    set(_tt_args)
    foreach(_elem ${IA_TARGET_TYPES})
      if(_elem IN_LIST _valid_target_types)
        ams_log(DEBUG "${CMAKE_CURRENT_FUNCTION}: Found valid target type: ${_elem}")
        set(_tt_name ${_elem})
        set(_tt_args)
      elseif(_tt_name AND _elem IN_LIST _valid_tt_args)
        if(_elem IN_LIST _valid_tt_args AND _elem IN_LIST _tt_args)
          ams_log(WARNING "${CMAKE_CURRENT_FUNCTION}: a redundant TARGET_TYPES argument '${_elem}' was specified for target type '${_tt_name}'")
        endif()
        list(APPEND _tt_args ${_elem})
      elseif(_tt_name)
        list(APPEND _tt_args ${_elem})
      else()
        ams_log(FATAL_ERROR "${CMAKE_CURRENT_FUNCTION}: TARGET_TYPES argument '${_elem}' is not valid without a target type")
      endif()
      if(_tt_name AND _tt_args)
        dict(_target_types SET KEY ${_tt_name} VALUE ${_tt_args} ERROR_VARIABLE _json_err)
        if(_json_err)
          ams_log(FATAL_ERROR "${CMAKE_CURRENT_FUNCTION}: Failed to set TARGET_TYPES: ${_json_err}")
        endif()
      endif()
    endforeach()
    dict(_output SET ERROR_VARIABLE _json_err KEY TARGET_TYPES VALUE ${_target_types})
    if(_json_err)
      ams_log(FATAL_ERROR "${CMAKE_CURRENT_FUNCTION}: Failed to set TARGET_TYPES: ${_json_err}")
    endif()
  endif()
  ...
  if(IA_INSTALL_FILE_SET)
    if(NOT IA_FILE_SET) # TODO: regex check
      ams_log(FATAL_ERROR "${CMAKE_CURRENT_FUNCTION}: INSTALL_FILE_SET specified but FILE_SET not specified or invalid.")
    endif()
    get_file_set_install_command_args(_fs_json FILE_SET_ARGS FILE_SET ${IA_FILE_SET})
    dict(_output GET _var_file_sets ERROR_VARIABLE _json_err KEY FILE_SETS)
    if(_json_err)
      list(APPEND _var_file_sets ${_fs_json})
      dict(_output SET ERROR_VARIABLE _json_err KEY FILE_SETS VALUE ${_var_file_sets})
      if(_json_err)
        ams_log(FATAL_ERROR "${CMAKE_CURRENT_FUNCTION}: Failed to set FILE_SETS: ${_json_err}")
      endif()
    else()
      list(APPEND _var_file_sets ${_fs_json})
      dict(_output SET ERROR_VARIABLE _json_err KEY FILE_SETS VALUE ${_var_file_sets})
      if(_json_err)
        ams_log(FATAL_ERROR "${CMAKE_CURRENT_FUNCTION}: Failed to append FILE_SETS: ${_json_err}")
      endif()
    endif()
  endif()
  if(IA_INSTALL)
    ...
    # TARGETS
    if(NOT _var_target)
      message(FATAL_ERROR "${CMAKE_CURRENT_FUNCTION}->INSTALL: TARGETS not specified")
    endif()
    list(APPEND _install_args TARGETS ${_var_target})
    message(DEBUG "${CMAKE_CURRENT_FUNCTION}: TARGETS: ${_var_target}")

    # EXPORT
    if(NOT _var_export)
      message(FATAL_ERROR "${CMAKE_CURRENT_FUNCTION}->INSTALL: EXPORT not specified")
    endif()
    list(APPEND _install_args EXPORT ${_var_export})
    message(DEBUG "${CMAKE_CURRENT_FUNCTION}: EXPORT: ${_var_export}")
    # COMPONENT
    if(_var_component)
      list(APPEND _install_args COMPONENT ${_var_component})
      message(DEBUG "${CMAKE_CURRENT_FUNCTION}: COMPONENT: ${_var_component}")
    endif()
    # TARGET_TYPES
    dict(_output GET _install_target_types KEY TARGET_TYPES ERROR_VARIABLE _json_err)
    if(_json_err)
      message(FATAL_ERROR "${CMAKE_CURRENT_FUNCTION}: Failed to get TARGET_TYPES: ${_json_err}")
    else()
      dict(_install_target_types KEYS _keys KEY TARGET_TYPES ERROR_VARIABLE _json_err)
      if(_json_err)
        message(FATAL_ERROR "${CMAKE_CURRENT_FUNCTION}: Failed to get TARGET_TYPES keys: ${_json_err}")
      endif()
      foreach(_key ${_keys})
        dict(_install_target_types GET _ttype KEY ${_key} ERROR_VARIABLE _json_err)
        if(_json_err)
          message(FATAL_ERROR "${CMAKE_CURRENT_FUNCTION}: Failed to get TARGET_TYPES key ${_key}: ${_json_err}")
        endif()
        list(APPEND _install_args ${_key} ${_ttype})
        message(DEBUG "${CMAKE_CURRENT_FUNCTION}: TARGET_TYPES: ${_key} ${_ttypes}")
      endforeach()
    endif()
    # FILE_SETS
    dict(_output GET _install_file_sets KEY FILE_SETS ERROR_VARIABLE _json_err)
    if(NOT _json_err)
      foreach(_fs ${_install_file_sets})
        message(DEBUG "${CMAKE_CURRENT_FUNCTION}: FILE_SETS: ${_fs}")
        dict(_fs GET _fs_name KEY FILE_SET ERROR_VARIABLE _json_err)
        if(_json_err)
          message(FATAL_ERROR "${CMAKE_CURRENT_FUNCTION}: Failed to get FILE_SET: ${_json_err}")
        else()
          list(APPEND _install_args FILE_SET ${_fs_name})
          message(DEBUG "${CMAKE_CURRENT_FUNCTION}: FILE_SET: ${_fs_name}")
        endif()
        dict(_fs GET _fs_dest KEY DESTINATION ERROR_VARIABLE _json_err)
        if(_json_err)
          message(FATAL_ERROR "${CMAKE_CURRENT_FUNCTION}: Failed to get DESTINATION: ${_json_err}")
        else()
          list(APPEND _install_args DESTINATION ${_fs_dest})
          message(DEBUG "${CMAKE_CURRENT_FUNCTION}: DESTINATION: ${_fs_dest}")
        endif()
      endforeach()
    endif()
    message(STATUS "${CMAKE_CURRENT_FUNCTION}: install(${_install_args})")
    set(_output ${_install_args})
  endif()
